...
function deps(decision: SecurityDecision = allow) {
  return {
    ...
    discord: {
      ...
      timeoutMember: vi.fn().mockResolvedValue(undefined),
    },
  };
}
describe('executeMemberTimeout', () => {
  it('rejects invalid duration before authorization or mutation', async () => {
    const d = deps();
    const result = await executeMemberTimeout({ ...input, duration: '29d' }, d);
    expect(result.executed).toBe(false);
    expect(result.content).toContain('Invalid timeout duration');
    expect(d.authorize).not.toHaveBeenCalled();
    expect(d.discord.timeoutMember).not.toHaveBeenCalled();
  });
  it('authorizes member.timeout, correlates, then mutates with parsed milliseconds', async () => {
    ...
    expect(d.discord.timeoutMember).toHaveBeenCalledWith({
      guildId: '100',
      targetUserId: '77',
    ...
    expect(d.correlations.create.mock.invocationCallOrder[0]).toBeLessThan(
      d.discord.timeoutMember.mock.invocationCallOrder[0]!,
    );
  });
  it('does not mutate a protected target denial', async () => {
    ...
    expect(d.correlations.create).not.toHaveBeenCalled();
    expect(d.discord.timeoutMember).not.toHaveBeenCalled();
  });
  it('sanitizes Discord mutation failure', async () => {
    const d = deps();
    d.discord.timeoutMember.mockRejectedValueOnce(new Error('token=secret'));
    const result = await executeMemberTimeout(input, d);
    expect(result.executed).toBe(false);
