import { type ActionPolicy } from '@knight/contracts';
...
function profile(id:string, rank:number) { return { id, guildId:'100', profileId:`p-${id}`, version:1, profileName:id, discordRoleId:`r-${id}`, rank, permissions:['member.unban'] as const, actionPolicies:{ 'member.unban': unlimited } }; }
const input={ guildId:'100', actorUserId:'42', targetUserId:'77', knightBotUserId:'999', reason:'appeal accepted', nowMs:10_000 } as const;
function deps(ownerId='1') { return {
  ...
  discord:{
    ...
    getMemberState:vi.fn().mockResolvedValue(null), unbanMember:vi.fn().mockResolvedValue(undefined),
  },
}; }
describe('executeMemberUnban',()=>{
  it('protects a higher-ranked banned staff member using durable Knight state',async()=>{
    const d=deps(); const result=await executeMemberUnban(input,d);
    expect(result.executed).toBe(false); expect(result.content).toContain('protected');
    expect(d.discord.getMemberState).toHaveBeenCalledWith('100','77'); expect(d.discord.unbanMember).not.toHaveBeenCalled();
  });
  it('lets the guild owner unban despite the target durable rank',async()=>{
    const d=deps('42'); d.staffProfiles.getEffectiveProfile.mockImplementation(async(_g:string,u:string)=>u==='77'?profile('target',99):null);
    const result=await executeMemberUnban(input,d); expect(result.executed).toBe(true);
    expect(d.correlations.create).toHaveBeenCalledTimes(1); expect(d.discord.unbanMember).toHaveBeenCalledWith({ guildId:'100', targetUserId:'77', reason:'appeal accepted' });
  });
});
