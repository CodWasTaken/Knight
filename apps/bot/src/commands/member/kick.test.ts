...
describe('executeMemberKick', () => {
  it('uses member.kick and correlates before Discord mutation', async () => {
    const d=deps(); const result=await executeMemberKick(input,d); expect(result.executed).toBe(true);
    expect(d.authorize).toHaveBeenCalledWith(expect.objectContaining({ action:'member.kick' }), expect.anything());
    expect(d.correlations.create.mock.invocationCallOrder[0]).toBeLessThan(d.discord.kickMember.mock.invocationCallOrder[0]!);
  });
  it('blocks rate-exhausted kicks without mutation', async () => {
    const d=deps({ ...allow, decision: PolicyDecision.Deny, code:'RATE_LIMIT_EXCEEDED', reason:'limited' });
    const result=await executeMemberKick(input,d); expect(result.executed).toBe(false); expect(result.content).toContain('rate limit'); expect(d.discord.kickMember).not.toHaveBeenCalled();
  });
});
