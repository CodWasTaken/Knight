...
`.dockerignore` excludes `.env` and `.env.*` from the image build context. Secrets are injected into containers at runtime. Keep `.env` readable only by trusted administrators and never commit it. Docker named volumes are still host-local data: include both `knight-postgres` and `knight-backups` in your normal encrypted off-host backup process if you need recovery after host loss.
